...
DEPLOY_CMD="${DOCMOLDER_VPS_DEPLOY_CMD:-sudo /usr/local/sbin/docmolder-update-vps}"
...
if [[ -z "${DOCMOLDER_VPS_SSH_KNOWN_HOSTS:-}" ]]; then
  echo "Missing DOCMOLDER_VPS_SSH_KNOWN_HOSTS" >&2
  exit 1
fi
...
set -euo pipefail
${DEPLOY_CMD@Q} deploy ${TARGET_REF@Q}
