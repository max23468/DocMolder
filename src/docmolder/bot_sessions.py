...
from docmolder.action_catalog import build_session_file, build_session_recap
...
def _purge_expired_sessions(deps: bot_runtime.BotDependencies) -> None:
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await message.reply_text(
        bot_admin._build_access_status_message(deps, user.id), reply_markup=build_main_menu_keyboard()
    )
...
def _build_job_queue_limit_message(max_active_jobs_per_user: int) -> str:
    return f"{JOB_QUEUE_LIMIT_MESSAGE} Limite attuale: {max_active_jobs_per_user} job attivi per utente. Prossimo passo: attendi l'ultimo risultato o usa /status."
    ...
    try:
        ...
        if pending_action in {
            ...
            if split_output_zip is None:
                await update.effective_message.reply_text(
                    bot_menu._build_split_output_prompt(user_id, deps),
...
async def _handle_pending_document_photo_mode_input(
    *,
    ...
    if mode is None:
        await update.effective_message.reply_text(
            bot_menu._build_document_photo_mode_prompt(), reply_markup=build_document_photo_mode_keyboard()
        )
        return True
    if not bot_jobs._has_capacity_for_new_job(user_id, deps):
        await update.effective_message.reply_text(
