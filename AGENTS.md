...
- Per modifiche minuscole e a basso rischio, chiaramente solo documentali o di istruzioni operative, evita la trafila lunga branch/PR/release se non aggiunge valore: stai su `main` aggiornato e usa `make publish-docs TITLE="docs: <descrizione>"`, che deve fare preflight/check mirati, commit diretto e push senza PR. Questa scorciatoia vale solo per `AGENTS.md`, `README.md` o `docs/**`, senza deploy/release attesi, e non vale per codice runtime, script, workflow CI, configurazione, deploy, dati, release-owned files o cambi ambigui.
