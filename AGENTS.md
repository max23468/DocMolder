...
- Quando inizi una nuova operazione GitHub, riprendi lavoro su una PR o fai il solito giro dei commenti bot, non limitarti alla PR corrente né alle sole PR aperte: verifica tutti i commenti/review bot o thread inline ancora in sospeso su tutte le PR del repository, indipendentemente dallo stato (`open`, `closed` o `merged`), incluse PR precedenti, diverse da quella in lavorazione, Release PR e PR storiche. Questo non significa revisionare o rileggere per intero tutte le PR: il controllo deve essere mirato solo alla presenza di commenti/thread aperti del bot. Usa `scripts/github_maintenance_report.py` o `make github-maintenance` come censimento iniziale quando utile, ma completa il controllo con una lista paginata di tutte le PR (`gh pr list --state all --limit <N>` o API equivalente) e poi `scripts/check_codex_bot_comments.py --pr <numero> --fail` sulle PR da verificare; se il controllo non copre davvero tutte le PR per limiti di API/paginazione, dichiara esplicitamente il limite e il rischio residuo. Includi sempre `chatgpt-codex-connector`/`chatgpt-codex-connector[bot]`/Codex connector bot.
- Evita controlli ripetuti inutili nello stesso giro operativo: tieni traccia in chat dei numeri PR già controllati, dello stato/esito e, quando disponibile, di head SHA o timestamp ultimo commento/review. Se una PR è già stata controllata almeno due volte di fila nello stesso giro con esito pulito e non risultano nuovi commit, review, commenti, riaperture o richiesta diretta di ricontrollo, saltala nei passaggi immediatamente successivi e dillo brevemente in chat per evitare loop. Ricontrolla invece ogni PR con attività nuova, esito precedente non pulito o segnale esplicito, anche se non è la PR corrente.
- Se trovi commenti bot aperti su PR diverse da quella in lavorazione, anche chiuse o mergeate da tempo, non ignorarli solo perché non appartengono alla PR corrente: se sono azionabili, implementa quanto segnalano nella PR corrente solo quando lo scope resta coerente, altrimenti apri una PR correttiva mirata o documenta esplicitamente il falso positivo/non azionabile; poi chiudi/elimina/risolvi il commento o thread quando possibile.
- Prima di togliere una PR da draft o mergiarla, esegui il controllo globale dei commenti bot e poi `scripts/check_codex_bot_comments.py --pr <numero> --fail` sulla PR corrente; se trova commenti aperti del Codex connector bot, fermati, implementali e ripeti i check prima del merge.
- Dopo che una PR viene marcata ready, dopo che i check GitHub diventano verdi e subito prima del merge, ripeti il controllo globale dei commenti bot: le review del Codex connector possono arrivare in ritardo rispetto ai primi check. Se il merge è già avvenuto e compare una review/commento bot tardivo sulla PR appena chiusa o su una PR precedente, considera il flusso non concluso: apri una PR correttiva mirata, implementa il rilievo oppure documenta esplicitamente perché non è azionabile, e ricontrolla il thread prima della risposta finale.
- Dopo il merge di una PR funzionale, ricontrolla la PR appena chiusa con `scripts/check_codex_bot_comments.py --pr <numero> --fail` e `gh pr view <numero> --json reviews,comments`, poi ripeti il controllo globale su tutte le PR del repository per intercettare commenti rimasti in sospeso su PR precedenti; quindi verifica webhook/deploy/release sulla VPS. Non dichiarare "pubblicato", "done" o "nessun commento aperto" finché questi controlli non sono stati eseguiti sull'ultimo stato disponibile.
