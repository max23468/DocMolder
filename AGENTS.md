...
- Per modifiche minuscole e a basso rischio, chiaramente solo documentali o di istruzioni operative, evita la trafila lunga branch/PR/release se non aggiunge valore: dopo preflight/check mirati, puoi pubblicare con un commit diretto e coeso su `main`, senza deploy VPS quando il diff non è deploy-relevant. Questa scorciatoia non vale per codice runtime, configurazione, deploy, dati, release-owned files o cambi ambigui.
