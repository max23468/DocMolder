...
- Quando inizi una nuova operazione GitHub, riprendi lavoro su una PR o fai il solito giro dei commenti bot, non limitarti alla PR corrente: controlla in generale se ci sono commenti/review bot o thread inline rimasti aperti su tutte le PR rilevanti, incluse PR aperte, Release PR e PR precedenti mergeate/chiuse di recente. Usa `scripts/github_maintenance_report.py` o `make github-maintenance` come censimento iniziale e poi `scripts/check_codex_bot_comments.py --pr <numero> --fail` sulle PR candidate; includi sempre `chatgpt-codex-connector`/`chatgpt-codex-connector[bot]`/Codex connector bot.
- Se trovi commenti bot aperti su PR precedenti, non ignorarli solo perché non appartengono alla PR in lavorazione: se sono azionabili, implementa quanto segnalano nella PR corrente solo quando lo scope resta coerente, altrimenti apri una PR correttiva mirata o documenta esplicitamente il falso positivo/non azionabile; poi chiudi/elimina/risolvi il commento o thread quando possibile.
