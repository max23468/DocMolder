...
- Quando inizi una nuova operazione GitHub, riprendi lavoro su una PR o fai il solito giro dei commenti bot, non limitarti alla PR corrente né alle sole PR aperte: controlla in generale se ci sono commenti/review bot o thread inline rimasti aperti su tutte le PR del repository, indipendentemente dallo stato (`open`, `merged` o `closed`), incluse Release PR e PR storiche. Questo non significa revisionare o rileggere per intero tutte le PR: il controllo deve essere mirato solo alla presenza di commenti/thread aperti del bot. Usa `scripts/github_maintenance_report.py` o `make github-maintenance` come censimento iniziale quando utile, ma completa il controllo con una lista paginata di tutte le PR (`gh pr list --state all --limit <N>` o API equivalente) e poi `scripts/check_codex_bot_comments.py --pr <numero> --fail` sulle PR candidate; se il controllo non copre davvero tutte le PR per limiti di API/paginazione, dichiara esplicitamente il limite e il rischio residuo. Includi sempre `chatgpt-codex-connector`/`chatgpt-codex-connector[bot]`/Codex connector bot.
- Evita controlli ripetuti inutili nello stesso giro operativo: tieni traccia in chat dei numeri PR già controllati, dello stato/esito e, quando disponibile, di head SHA o timestamp ultimo commento/review. Se una PR è già stata controllata almeno due volte nello stesso giro con esito pulito e non risultano nuovi commit, review o commenti dopo l'ultimo controllo, non ricontrollarla ancora solo per scrupolo; ricontrolla invece sempre la PR corrente dopo push/ready/check verdi e qualsiasi PR che abbia attività nuova o un esito precedente non pulito.
