...
- Coverage: sulla base locale `c11ed5958c01`, dopo lo slice offline access/policy di `src/docmolder/bot.py`, la coverage totale misurata è `87.3%` contro target `90%`; prossimo step consigliato: coprire altri rami offline di `bot.py` su command/callback handler e pending input, senza coinvolgere Telegram live o VPS.
