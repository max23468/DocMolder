...
| --- | --- | --- | --- | --- |
| in corso | Codex preserve webhook worker flag | codex/preserve-webhook-worker-flag | `deploy/update-vps.sh`, test deploy script | Follow-up P1 tardivo su PR #100: preservare env worker/marker attraverso `sudo` prima di reinstallare il listener webhook. |
