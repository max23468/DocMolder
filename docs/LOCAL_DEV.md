...
Opzionale per lo sblocco Excel `.xls`:
...
Il controllo blocca i casi che rendono rumorosa la pubblicazione: `HEAD detached`, branch indietro o divergente da `origin/main`, file riservati al versioning (`CHANGELOG.md`, `pyproject.toml`, `src/docmolder/__init__.py`) e run GitHub fallite sullo SHA corrente. Con Actions riattivate resta il primo guardrail economico locale; su GitHub la PR non draft verso `main` esegue anche `CI result`.
